...
3.2.3 节指出，快速重路由等复杂场景的测试需要控制面配置、外部操作和数据面报文在统一时间线上协调执行。本节介绍 P4-BISG 输出的测试用例结构及其核心组件——统一执行时间线的形式化定义与组装方法。
...
该结构的设计目标不是方便阅读，而是保证生成结果具备明确的执行语义，能够被直接回放。
...
为直观说明统一执行时间线如何编排不同类型的操作，表 3-3 以三个代表性场景为例，展示其执行时间线的结构。

**表 3-3 三个场景的执行时间线结构示例**
...
**图 3-5 三个场景的统一执行时间线对比图（占位）**

> 图注说明：三列对比图，分别展示 Stateful Firewall / Link Monitor / Fast Reroute 三个场景的 execution_sequence 时间线。每列从上到下为时间推进方向，不同操作类型用不同色块区分（蓝色 = control_plane_config，橙色 = operator_action，绿色 = send_packet）。时间线旁标注关键操作的简要描述。
>
> 绘图风格参考 Confucius Figure 1（Capacity What-if DAG）：清晰的节点排列，色彩区分操作类型，整体美观且信息密度高。

### （5）产物落盘与可追溯记录

为支持实验复现与失败定位，P4-BISG 对每次生成过程落盘两类产物：
