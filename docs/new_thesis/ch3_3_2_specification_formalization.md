...
P4LTL 时序规范以声明式方式描述了待验证的跨包时序性质，但可执行测试种子需要精确的场景结构、角色绑定、时序步骤和观测标准。为弥合二者之间的结构性语义鸿沟，P4-BISG 引入结构化任务规约对象 TaskSpec 作为规范与执行之间的桥梁。本节首先给出规范语义规约问题的形式化定义，然后说明 P4LTL 规范的结构化分解方法，最后给出规约算法。
...
场景契约 $\Lambda$ 是 TaskSpec 中承上启下的核心组件。它将 P4LTL 规范中隐含的行为结构显式化为可校验的场景-步骤层次。
...
**图 3-2 P4LTL 规范到 TaskSpec 的结构化分解过程（占位）**

> 图注说明：图分三列。左列展示一个 P4LTL 规范示例（包含 `#LTLProperty`、`#LTLFairness`、`#LTLVariables`），中间列展示三个维度的分解过程（时序逻辑分离→控制面约束提取→数据面特征映射），右列展示生成的 TaskSpec 六元组各分量。从左到右用箭头标注哪些 P4LTL 元素映射到 TaskSpec 的哪些分量。底部标注 ProgramContext 的工具查询接口。
>
> 绘图风格参考 Confucius Figure 8（Analects composition）：自左向右的数据流，节点用不同形状和颜色区分 P4LTL 元素/分解过程/TaskSpec 输出，旁边用小字标注具体示例（如 Firewall 的 `direction == 0` 映射为 internal 角色）。
