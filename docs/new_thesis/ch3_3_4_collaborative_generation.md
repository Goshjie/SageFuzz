...
3.2.3 节通过快速重路由场景表明，可执行测试种子涉及数据面报文、控制面规则、外部操作和观测预言机等多个维度的工件，这些工件必须在语义上相互一致。为解决这一多维联合一致性问题，P4-BISG 将生成过程拆分为多个角色明确的阶段，并在每个阶段设置确定性校验门控。本节首先给出确定性校验谓词的形式化定义，然后说明多阶段协同生成的编排算法，最后介绍回退机制。

### （1）确定性校验谓词

P4-BISG 的正确性保证不依赖于生成模型的一次性准确输出，而是建立在阶段间的确定性校验之上。系统对三类核心工件定义了形式化的校验谓词。
...
算法 3-3 给出了 P4-BISG 的多阶段协同生成与门控调度流程。算法接收经过规约的 TaskSpec 和程序上下文，逐阶段生成报文序列、控制面规则和预言机预测，每个阶段在生成后立即执行确定性校验。

---

**算法 3-3. 多阶段协同生成与门控调度算法**
...
3. **始终保证可落盘。** 回退策略的目标是保证 testcase 可序列化、可回放、可追溯，即使语义上不完美，也能为后续的人工分析或迭代优化提供基础。

**图 3-4 多阶段协同生成流水线与门控反馈路径（占位）**

> 图注说明：图中横向展示生成流水线的四个阶段（A: 报文序列 → B: 控制面规则 → C: 预言机 → D: 执行序列），每个阶段内部用"生成→校验→通过/回退"的菱形判断分支表示。实线箭头表示正向数据流，虚线/红色箭头表示修复反馈回路和最小回退路径。校验门控节点旁标注对应的校验谓词（$V_{\text{packet}}$、$V_{\text{entity}}$、$V_{\text{exec}}$）。最终输出汇聚到 testcase 组装节点。
>
> 绘图风格参考 Confucius Figure 2（Fault Diagnosis 流程图）：带分支判断的多步骤流程，用颜色区分正常路径（蓝/绿）和回退路径（红/橙），整体呈现左到右的流水线布局。
